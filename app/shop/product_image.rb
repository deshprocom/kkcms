...
ActiveAdmin.register Shop::Image, as: 'Image', namespace: :shop do
  ...
  member_action :reposition, method: :post do
    blind = Shop::Image.find(params[:id])
    next_blind = params[:next_id] && Shop::Image.find(params[:next_id].split('_').last)
    prev_blind = params[:prev_id] && Shop::Image.find(params[:prev_id].split('_').last)
